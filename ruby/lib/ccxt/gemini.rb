...
module Ccxt
  class Gemini < Exchange
    ...
    async def fetch_order_book(symbol, limit = nil, params = {})
      ...
      response = await{ self.publicGetBookSymbol(self.shallow_extend(request, params)) }
    ...
    async def fetch_ticker(symbol, params = {})
      ...
      return {
        ...
        'percentage' => nil,
        'average' => nil,
        'baseVolume' => ticker['volume'][baseVolume].to_f,
        'quoteVolume' => ticker['volume'][quoteVolume].to_f,
        'info' => ticker
      }
    end
    ...
    async def fetch_trades(symbol, since = nil, limit = nil, params = {})
      await{ self.load_markets }
      market = self.market(symbol)
      response = await{ self.publicGetTradesSymbol(self.shallow_extend({
        'symbol' => market['id']
      }, params)) }
    ...
    async def create_order(symbol, type, side, amount, price = nil, params = {})
      ...
      response = await{ self.privatePostOrderNew(self.shallow_extend(order, params)) }
      return {
        'info' => response,
    ...
    async def withdraw(code, amount, address, tag = nil, params = {})
      ...
      response = await{ self.privatePostWithdrawCurrency(self.shallow_extend({
    ...
    async def fetch_transactions(code = nil, since = nil, limit = nil, params = {})
      await{ self.load_markets }
      request = {}
      response = await{ self.privatePostTransfers(self.shallow_extend(request, params)) }
    ...
    async def create_deposit_address(code, params = {})
      await{ self.load_markets }
      currency = self.currency(code)
      response = await{ self.privatePostDepositCurrencyNewAddress(self.shallow_extend({
        'currency' => currency['id']
      }, params)) }
