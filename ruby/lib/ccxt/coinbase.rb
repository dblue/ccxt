...
module Ccxt
  class Coinbase < Exchange
    ...
    async def fetch_my_sells(symbol = nil, since = nil, limit = nil, params = {})
      ...
      sells = await{ self.privateGetAccountsAccountIdSells(self.shallow_extend({
        'account_id' => accountId
      }, query)) }
      return self.parse_trades(sells['data'], nil, since, limit)
    end

    async def fetch_my_buys(symbol = nil, since = nil, limit = nil, params = {})
      # they don't have an endpoint for all historical trades
      accountId = self.safe_string_2(params, 'account_id', 'accountId')
      if accountId.nil?
        raise(ArgumentsRequired, self.id + ' fetchMyTrades requires an account_id or accountId extra parameter, use fetchAccounts or loadAccounts to get ids of all your accounts.')
      ...
      await{ self.load_markets }
      query = self.omit(params, ['account_id', 'accountId'])
      buys = await{ self.privateGetAccountsAccountIdBuys(self.shallow_extend({
        'account_id' => accountId
      }, query)) }
      return self.parse_trades(buys['data'], nil, since, limit)
    end

    async def fetch_transactions_with_method(method, code = nil, since = nil, limit = nil, params = {})
      accountId = self.safe_string_2(params, 'account_id', 'accountId')
      if accountId.nil?
      ...
      await{ self.load_markets }
      query = self.omit(params, ['account_id', 'accountId'])
      response = await{ self.send_wrapper(method, self.shallow_extend({
        'account_id' => accountId
      }, query)) }
