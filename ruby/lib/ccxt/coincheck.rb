...
module Ccxt
  class Coincheck < Exchange
    ...
    async def fetch_balance(params = {})
      ...
      for i in (0...currencies.length)
        ...
        if balances.include?(lowercase)
          account['free'] = balances[lowercase].to_f
        reserved = lowercase + '_reserved'
        if balances.include?(reserved)
          account['used'] = balances[reserved].to_f
    ...
    async def fetch_my_trades(symbol = nil, since = nil, limit = nil, params = {})
      market = self.market(symbol)
      response = await{ self.privateGetExchangeOrdersTransactions(self.shallow_extend({}, params)) }
    ...
    async def create_order(symbol, type, side, amount, price = nil, params = {})
      ...
      response = await{ self.privatePostExchangeOrders(self.shallow_extend(order, params)) }
      return {
        'info' => response,
        'id' => response['id'].to_s
      }
